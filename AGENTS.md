...
- 本机通常通过 `xray` 代理访问 GitHub，当前系统代理地址为 `127.0.0.1:10808`。执行 `clone`、`fetch`、`pull`、`push` 等 GitHub 网络操作前，必须先确认系统代理配置，并验证对应本地端口正在监听。
- 若代理地址或端口与当前记录不一致、端口未监听或连接失败，必须停止并询问用户；不得擅自修改全局 Git 代理配置。
